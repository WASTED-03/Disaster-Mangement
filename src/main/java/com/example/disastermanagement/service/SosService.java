...
public class SosService {
    ...
    public SosService(SosRepository repo, NotificationService notificationService) {
        this.repo = repo;
    ...
    public SosRequest createSos(SosRequest sos) {
        ...
        SosRequest savedSos = repo.save(sos);
        
        // STEP 12.5: Send real-time notifications after saving SOS
        try {
            // Notify all admins about new SOS request
            String adminMessage = String.format("New SOS request created by %s at (%.4f, %.4f). Type: %s",
                    savedSos.getUserEmail(), savedSos.getLatitude(), savedSos.getLongitude(), 
                    savedSos.getType() != null ? savedSos.getType() : "Unknown");
            notificationService.notifyAdmins(adminMessage);
            
            // Notify the user that their SOS was logged successfully
            notificationService.notifyUser(savedSos.getUserEmail(), "SOS logged successfully. Help is on the way!");
        } catch (Exception e) {
            // Log error but don't fail the SOS creation
            System.err.println("Error sending SOS notifications: " + e.getMessage());
        }
